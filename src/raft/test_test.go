...
func TestReElection2A(t *testing.T) {
	...
	Logf("test after disconnect server:%d\n", leader1)
	cfg.disconnect(leader1)
	cfg.checkOneLeader()

	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	Logf("test after connect leader:%d\n", leader1)
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()
	//fmt.Println("test after old leader rejoin")
	// if there's no quorum, no leader should
	// be elected.
	Logf("disconnect server:[%d, %d, %d]\n", leader2, (leader2 + 1) % servers)
	cfg.disconnect(leader2)
	cfg.disconnect((leader2 + 1) % servers)
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()
	//fmt.Println("test after No leader")
	// if a quorum arises, it should elect a leader.
	Logf("connect server: %d\n", (leader2 + 1) % servers)
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()
	//fmt.Println("test after quorum:")
...
func TestFailAgree2B(t *testing.T) {
	...
	Logf("Reconnected the disconnect server")
...
func TestRejoin2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	cfg.disconnect(leader1)
	Logf("disconnected old leader (server %d)\n", leader1)
	...
	leader2 := cfg.checkOneLeader()
	cfg.disconnect(leader2)
	Logf("disconnected server %d\n", leader2)
	// old leader connected again
	cfg.connect(leader1)
	Logf("reconnected old leader(server %d)\n", leader1)

	cfg.one(104, 2, true)

	// all together now
	cfg.connect(leader2)
	Logf("reconnected server %d\n", leader2)
...
func TestBackup2B(t *testing.T) {
	...
	Logf("disconnected server[%d,%d,%d]\n",
	...
	Logf("after submit lots of commands that won't commit\n")
	...
	Logf("disconnect server[%d, %d], connect server[%d, %d, %d]\n",
		(leader1 + 0) % servers, (leader1 + 1) % servers, (leader1 + 2) % servers, (leader1 + 3) % servers, (leader1 + 4) % servers)
	// lots of successful commands to new group.
	for i := 0; i < 50; i++ {
		cfg.one(rand.Int(), 3, true)
	}
	Logf("1st lots of successful commands to new group\n")
	...
	Logf("lots more commands to server %d that won't commit.\n", leader2)
	time.Sleep(RaftElectionTimeout / 2)
	...
	cfg.connect(other)
	Logf("disconnect all server, connect server[%d, %d, %d]\n",
	...
	Logf("2nd lots of successful commands to new group\n")
	// now everyone
	for i := 0; i < servers; i++ {
		cfg.connect(i)
	}
	Logf("now connect everyone\n")
	cfg.one(rand.Int(), servers, true)
